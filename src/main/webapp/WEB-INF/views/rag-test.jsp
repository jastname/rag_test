...
<body>
    ...
    <form id="ragForm" action="#" method="post" novalidate onsubmit="return false;">
        ...
        function renderReferences(references) {
            ...
            referencesContainer.innerHTML = references.map(function(ref, index) {
                const sourceUrl = ref.sourceUrl
                    ? '<p><strong>sourceUrl:</strong> <a href="' + escapeHtml(ref.sourceUrl) + '" target="_blank" rel="noreferrer noopener">' + escapeHtml(ref.sourceUrl) + '</a></p>'
                    : '';

                return ''
                    + '<div class="reference">'
                    + '  <h3>#' + (index + 1) + ' ' + escapeHtml(ref.title || '') + '</h3>'
                    + '  <p><strong>storyId:</strong> ' + escapeHtml(ref.storyId) + ' / <strong>chunkId:</strong> ' + escapeHtml(ref.chunkId) + ' / <strong>chunkIndex:</strong> ' + escapeHtml(ref.chunkIndex) + '</p>'
                    + '  <p><strong>similarity:</strong> ' + escapeHtml(ref.similarity) + '</p>'
                    + sourceUrl
                    + '  <pre>' + escapeHtml(ref.chunkText || '') + '</pre>'
                    + '</div>';
            }).join('');
        }
